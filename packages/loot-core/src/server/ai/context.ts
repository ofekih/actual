...
export async function getTaxonomies(): Promise<TaxonomyContext> {
  ...
  const { data: cspGroups } = (await aqlQuery(
    q('csp_category_groups').select('*'),
  )) as { data: Array<{ id: string; name: string }> };
  const { data: cspCategories } = (await aqlQuery(
    q('csp_categories').select('*'),
  )) as { data: Array<{ id: string; name: string; group?: string }> };
